...
TASKS = [
    {
        "node": "bot.szspd.cn",
        "task_id": "recovery-bot.szspd.cn-1772534013",
        "description": "节点 bot.szspd.cn 自我恢复"
    },
    {
        "node": "bot2.szspd.cn", 
        "task_id": "recovery-bot2.szspd.cn-1772534023",
        "description": "节点 bot2.szspd.cn 自我恢复"
    }
]
...
def recover_task(task_info):
    ...
    log("=" * 50)
    log(f"开始恢复任务: {description} ({task_id})")
    log("=" * 50)
    
    # 1. 检查SSH连接
    if not check_ssh_connectivity(node):
        log(f"❌ 无法建立SSH连接，任务 {task_id} 恢复失败", "ERROR")
        return False
...
def main():
    ...
    for task_info in TASKS:
        try:
            if recover_task(task_info):
                success_count += 1
            else:
                failed_count += 1
        except Exception as e:
            log(f"❌ 任务恢复过程中出错 - {task_info['task_id']}: {e}", "ERROR")
            failed_count += 1
    ...
    log("=" * 50)
    log(f"任务恢复完成 - 成功: {success_count}, 失败: {failed_count}")
    log("=" * 50)
